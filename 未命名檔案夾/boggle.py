...
def search(x_y_dic, neighbor_dic, dic):
	...
	:param dic: data load from dictionary
	"""
	start_list = []  # record characters that had become initial character once
	word_result = []  # words that were found from boggle
	for i in range(4):
		for j in range(4):
			start_position = (i, j)
...
def search_helper(x_y_dic, neighbor_dic, start_position, start_list, candidate_list, dic, word_result):
	...
	:param candidate_list: combination of character, it could be a existing word
	:param dic: data load from dictionary
	:param word_result: words that were found from boggle
	"""
	if len(start_list) == len(x_y_dic):
...
def set_x_y(input_dic):
	...
	for key, value in input_dic.items():
		...
		for j in range(len(new)):
			x_y_dic[(j, int(key))] = new[j]  # x_y_dic looks like --> (x,y):'ch'
	return x_y_dic
...
def find_neighbor():
	...
	for x in range(4):
		for y in range(4):
			...
			neighbor_dic[(x, y)] = search_lst  # neighbor_dic looks like --> (x,y):[neighbor]
	return neighbor_dic
